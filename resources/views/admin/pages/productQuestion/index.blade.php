                     ...
                     @foreach($productQuestion as $item)
                        <tr>
                           ...
                           @switch($item->status)
                              @case(0)
                              @php
                                 $url = route('productQuestion.status', $item->id);
                           ...
                           <td>{!! $status !!}</td>
                           <td>
                              <form action="{{ route('productQuestion.destroy', $item->id) }}" method="post" class="myForm">
                                 @csrf
                                 <button class="btn btn-danger">حذف</button>
                              </form>
                           </td>
                           <td><a href="{{ route('productQuestion.show', $item->id) }}" class="btn btn-primary">مشاهده</a>
                           </td>
                        </tr>
                     @endforeach
                     </tbody>
